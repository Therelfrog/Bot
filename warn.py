...
class WarnSystem(commands.Cog):
    ...
    async def warn(self, ctx, user: discord.Member, *, reason: str):
        try:
            ...
            embed = discord.Embed(title="Warnung erteilt", color=discord.Color.brand_red())
    ...
    @commands.has_permissions(administrator=True)
    async def set_punishment(self, ctx, warn_count: int, punishment_type: Option(str, "kick, ban, mute, none"),
                             duration: int = 0):
        try:
    ...
    async def show_punishments(self, ctx):
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute(
                        'SELECT warn_count, punishment_type, duration FROM punishment_settings ORDER BY warn_count') as cursor:
